...
const SearchPage = () => {
  ...
  const handleFromSubmit = (e) => {
    ...
    if (filteredDonors == 0) {
      toast.error("Sorry, we couldn’t find any results for your input");
    }
      ...
      <div className=" pt-1 rounded-xl">
        <div className="bg-white px-4 py-6 flex flex-col justify-center items-center">
          ...
          <div className="p-4 border-b-4 border-l-4  border-red-500 rounded-b-2xl rounded-t-2xl">
            <h2 className=" text-2xl md:text-3xl text-red-500 font-semibold">
              Blood Donor Search
            </h2>
            <form onSubmit={handleFromSubmit}>
              <fieldset className="fieldset  pt-2 w-xs pr-2   lg:w-lg  font-medium">
                ...
                <select
                  ...
                  required
                  className="w-full p-3 border-b-2 border-gray-500 hover:border-red-600 outline-none bg-white text-gray-800"
                >
              ...
              <fieldset className="fieldset w-xs pr-2 pt-2 lg:w-lg  font-medium">
                ...
                <select
                  ...
                  required
                  className="w-full p-3 border-b-2 border-gray-500 hover:border-red-500 outline-none bg-white text-gray-800"
                >
              ...
              <div className="flex justify-end">
                <button className="px-8 mt-4  rounded cursor-pointer py-2 bg-red-500 text-white">
                  Search
                </button>
              </div>
            </form>
        ...
        <div>
          ...
          {searchResult.length === 0 ? (
            <>
              <div className="py-5 text-center text-gray-600 border-2 border-[#D25D5D] my-5 md:w-7/12 rounded mx-auto">
                <p>
                  Search for donors near you by blood group, district, <br />{" "}
                  and upazila — help someone live today
                </p>
              </div>
            </>
          ) : (
            <>
              <h2 className="text-center text-2xl md:text-4xl text-[#D25D5D]">
                Matching Donors
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 py-6">
                {searchResult?.map((user) => (
                  <SearchCard user={user} key={user?._id}></SearchCard>
                ))}
              </div>
            </>
          )}
